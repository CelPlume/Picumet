// EvilCharts（MIT）registry 组件，来源 https://evilcharts.com/docs/recharts/installation 的
// "recharts-area-chart" item（上游路径 src/registry/charts/recharts-area-chart.tsx）。本地改动三处：
//   1) 去掉 Next.js 的 "use client"（本项目是 Vite SPA，无 RSC）；
//   2) 导入路径 @/registry/ui/* → ../ui/*；
//   3) 上游使用 React 19 的 use() 与 <Context value> 写法，本项目锁在 React 18.3，
//      改写为 useContext() 与 <Context.Provider value>（仅此一处语义等价改写）。
import {
  Children,
  createContext,
  isValidElement,
...
import {
  ...
  type BrushProps,
  type EvilBrushRange,
} from "../ui/recharts-brush";
import {
  ChartLegend,
  ChartLegendContent,
  type ChartLegendVariant,
} from "../ui/recharts-legend";
import { ChartDot, type DotVariant } from "../ui/recharts-dot";

// EvilCharts（MIT）registry 组件，来源 https://evilcharts.com/docs/recharts/installation 的 "recharts-tooltip" item
// （上游路径 src/registry/ui/recharts-tooltip.tsx）。本地改动仅两处：
//   1) 去掉 Next.js 的 "use client"（本项目是 Vite SPA，无 RSC）；
//   2) 导入路径 @/registry/ui/* → 同目录相对路径。
// 其余实现与上游逐行一致，便于日后按 registry 同步。
import { getPayloadConfigFromPayload, getColorsCount, useChart } from "../ui/recharts-chart";

  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      ...
      <Card className="mt-3 min-h-0 flex-1 overflow-y-auto overflow-x-auto">
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b text-left text-muted-foreground">
              <th className="px-4 py-2"><SortableHeader title="操作" sortKey="action" sort={sort} order={order} onSort={(k)=>{setSort(k);setOrder(order==='asc'?'desc':'asc');}} /></th>
              <th className="px-4 py-2">路径</th>
              <th className="px-4 py-2">用户</th>
              <th className="hidden px-4 py-2 md:table-cell">IP</th>
              <th className="px-4 py-2"><SortableHeader title="字节" sortKey="bytesTransferred" sort={sort} order={order} onSort={(k)=>{setSort(k);setOrder(order==='asc'?'desc':'asc');}} /></th>
              <th className="px-4 py-2"><SortableHeader title="时间" sortKey="createdAt" sort={sort} order={order} onSort={(k)=>{setSort(k);setOrder(order==='asc'?'desc':'asc');}} /></th>
            </tr>
          </thead>
          <tbody>
            {loading ? (
              ...
              <tr key={l.id} className="border-b last:border-0 hover:bg-accent/50">
                ...
                <td className="px-4 py-2 text-xs text-muted-foreground">{l.userId ? l.userId.slice(0, 8) : '匿名'}</td>
        ...
        {!loading && logs.length === 0 && <p className="py-8 text-center text-sm text-muted-foreground">暂无日志</p>}
      </Card>

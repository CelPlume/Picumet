...
管理端「系统设置」把限流写清楚了：开关旁直接给出每 IP 每分钟请求数（输入框）与生效规则说明——已登录用户按 2 倍、登录/注册等认证接口 5 次/分钟、自由模式按会话 60 / 按用户 120 次/分钟；同一处还有**下载限速**（默认 120 次/分钟，0 = 不限，只统计下载类请求）与**同时传输上限**（默认 4，0 = 不限），按用户（未登录按 IP）限制上传与下载网关的在途请求数，超限返回 429。
...
管理端「全部文件」表格新增**可见性列**（私密/站内/公开三态徽章 + 图标，公开待审核另显审核态），每行提供**属性设置**弹窗（名称、自定义标题、图标、颜色、可见性、游客可见性、访问密码，以及文件夹的**「同时应用到子项」**开关——默认不勾选，避免「点一个公开连带一串公开」）。「分享管理」表格补齐创建者、访问权限、密码保护、预览/下载开关、访问与下载计数、过期与状态列，并新增**分享设置（管理员）**弹窗：可改状态（有效/撤销）、有效期、最大访问/下载次数、预览/下载开关、访问权限与指定用户、重置或清除密码，只读展示分享项目、创建者与计数。

设置布局（`/settings/*`）左侧是纵向导航，包含个性化设置、API 密钥、访问规则。访问规则页列出用户以自己身份创建的规则（作用对象、效果、目标、权限），支持二次确认后撤销。管理后台（`/admin`）是两栏结构，左侧纵向导航、右侧内容区，手机上导航堆在内容上方。管理页面包括带统计卡片的仪表板、用户管理（编辑弹窗含角色/状态/能力位勾选与默认存储位置/存储上限/文件数量，宽屏响应式；工具栏「默认用户设置」按角色设置默认存储与限额，支持新建/删除角色，保存后覆盖该角色下全部用户）、存储配置（提供商对话框用预设下拉，R2/AWS S3/Oracle/MinIO/自定义，只预填字段，另带挂载路径）、权限规则（表格带“来源”列区分管理员规则与用户自建规则）、分享管理、全部文件（带可见性与审核状态列，可批准/驳回或直接调整可见性）、访问日志和系统设置。挂载点表单的「自动跨桶同步」开关属计划项——仅 Go 后端环境可勾选，当前 Workers（Node）后端不支持该能力，界面按禁用态呈现；挂载点即目录——新建挂载点后父目录列表里立即出现同名文件夹（行内菜单的改名/删除被拒，提示去存储配置里操作），改挂载路径会迁移该目录行、删除挂载点会清理它
...
- **表头形态统一**：文件页列表视图表头与后台/设置各表表头使用同一常量与同一几何（实测两侧同为 `font-size 14px`、`padding 8px 16px`、`border-radius 0`、底色 `rgba(255,255,255,0.45)`、`blur(16px)`、`position sticky`），文件页列表行内边距同步改为 `px-4` 以便与表头列对齐。

**粘性表头**：所有数据表（后台用户/全部文件/分享/日志/权限/存储提供商与挂载、设置的分享/API 密钥/访问规则）的表头在内容滚动时保持贴顶（`TABLE_HEAD_CLASS = sticky top-0 z-10 glass-header` + 4% 压暗叠加，集中在 `components/ui/sortable-header.tsx`，各页不再内联样式）。表头额外叠一层 4% 黑色 `background-image`（与文件项 hover 同一手法）：浅色壁纸下纯玻璃几乎看不出层次，加这层极浅 scrim 后表头带明显可辨（实测表头 `rgba(255,255,255,0.45)` / 数据行 `0.60`，视觉上表头为浅灰、数据行为白）。`.glass-header` 是比卡片低一档透明度的表面配方（`max(0.45, --glass-alpha - 0.28)`），因此滚动内容会在表头下方透出、模糊可见；同样消费 `--glass-alpha` / `--glass-blur` 三档门控（off 档实底并由 `.no-blur` 关停模糊），不会随行一起滑走；设置页表格改为有界滚动区（`max-h-[calc(100vh-16rem)] overflow-auto`）以便表头真正生效。
- **文件页列表视图**也有表头：名称/大小/修改时间三列可点击排序，表头贴在顶栏下方（`top-14`），滚动文件列表时不随内容移动。
- **文件页分页**：卡片与列表两种视图共用底部分页组件（`Pagination`），服务端分页 `limit` 默认 **50 条/页**，可切 20/100；换目录、改搜索词或排序后自动回到第 1 页。
- **卡片每行数量**：文件页卡片视图默认**每行 6 个**（移动端 2 个），可在「个性化设置」用滑块在 **4–8** 之间调整（`FilesPerRowSlider`，写入 `picumet:appearance.filesPerRow`，改动即时生效并本地持久化）。
...
- **日志表格高度**：日志页搜索/筛选栏占纵向空间，其表格卡片的相对最大高度单独调高（`max-h-[calc(100vh-9rem)]`，其余表格为 `-14rem`），可视区与全部文件页相当。

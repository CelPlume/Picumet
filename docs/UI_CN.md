...
- 顶部栏固定在最上方，包含 Logo、左上角标题（站点设置可留空 = 只显示 Logo，独立于浏览器标签页标题）、主导航，右侧依次是主题切换、语言切换和用户菜单。
- 顶部栏下方是公告横幅。
- 内容区居中，最大宽度 `1400px`。
...
**仪表板**（`/admin`）顶部是四张单行统计卡（图标 + 标签/数值堆叠 + 右侧副文本）：用户综述（管理员/用户/访客计数）、内存占用（容量未设置时仅显示数值，有值时附全宽细进度条）、桶数量、活跃挂载点；底部左侧是**桶泳道图**——每个存储桶一条泳道，挂载点与文件夹按列展开（每层一列、圆角连线、桶基色按层级色相旋转；锚点折叠空心、展开实心、无白色外圈），展开的挂载点下方是顶层文件夹行与递归文件计数（只显示计数、按该桶过滤），桶可折叠（默认展开）、挂载点可同时展开多个；某桶在某挂载点上是零文件备用时显示**备用桶 badge**，点击后高亮该挂载点主桶泳道 3 秒；右侧仍是**存储使用**（每挂载点用量与文件数，带容量进度条）与**近期活动**（最近 6 条）。
...
- 菜单弹出时**逐项入场**（动画档 `all`）：`.animate-dropdown` 直接子项依次淡入，步长与封顶见「入场动画体系」——新菜单项直接作为容器子元素渲染即可获得该效果，无需额外类。
...
- **卡片内部尽量不再用卡片**：卡内分区一律用**分隔线**（`border-t pt-3`，如设置页 SMTP 卡的测试邮箱；并列多段用容器 `divide-y` + 子项 `pt-4 first:pt-0`，如仪表盘趋势卡的三张图），需要小节标题时在分隔线下加一行 `text-sm font-medium`（如速率限制）。
- 禁止在 `Card` 内再嵌 `rounded-md border p-3` 一类的边框盒/子卡：玻璃卡片里叠边框盒既加重视觉噪音，也与「玻璃嵌套复合成不透明白板」的历史教训同源（见 `TableSkeleton` 例外一节）。
- 卡片的层级语义：`Card` 只属于页面顶层的功能分区；卡内的逻辑分组（如设置页的「直链挂载点」「速率限制」、趋势卡的每条曲线）一律降级为分隔线分区，不再争夺卡片外观。
...
- 列布局：每层一列（列宽 22px，第 n 列锚点圆心 x = 10 + 22n；行内容左内边距 = 圆心 + 12），层级为 桶 0 / 挂载点 1 / 文件夹 2 / 文件 3+。
- 锚点：桶 / 挂载点 / 文件夹折叠空心（`border-2`）、展开实心，无白色外圈，空心圆内绝不穿线（父列竖线止于转角起点）；叶子（文件、仪表盘计数行）用 4px 小实心点。
- 连线：转角是定高 SVG，起点从父列干线「长出来」（起点与干线描边融合 1px、切线竖直相切无拐点），三次曲线缓出后水平切入锚点左缘（C 控制点把弯曲摊开，竖直贴干段仅 2-3px，禁止四分之一圆的生硬直角感与长距离叠画干线）；祖先列只要「后面还有兄弟」就整行贯通（`pass` 布尔数组随递归下传），故展开子树不会把桶干线切断——竖线用 `top-0 height:100%` / `calc(50% - 10px)` 百分比表达，行高随内容与字号自适应。
- 配色：桶基色按层级做色相旋转（level 0 = 桶基色即其 pill 色，level n = 基色 + 34n°），同桶同族、层级可分辨。
- 桶可折叠（默认展开）、挂载点同时展开（`Set`），备用桶 badge 高亮主桶泳道 3 秒；挂载点视图根容器留 16px 内边距（不贴卡片边）。备用占位行（琥珀空心环）**同样接桶干线**——琥珀分支 + 不可展开与真实挂载点行区分；只有 badge 行与「无挂载点」提示对齐 level-1 内容列且不画连线。卡片底部不放「在用桶/备用桶」图例（配色已自释，图例随旧配色废弃删除）。
...
- **宽表横向滚动**：名称/路径等弹性列给 `minmax(≥140px, Nfr)` 下限——窄容器时网格整体溢出为横向滚动，而不是 `minmax(0,·)` 把列压成 0px 让内容叠上邻列；表头行 `whitespace-nowrap` 保持单行，表体 `onScroll` 时表头 `translateX(-scrollLeft)` 同步平移，横滚中列永不错位（已覆盖：全部文件、日志审计）。
- **表头字重**：全站表头文字一律 **700**（th 默认继承；`SortableHeader` 不再自带 `font-medium`，各页面也不得在 th 上另加 `font-medium`）。

**表头贴顶**：双表结构的表头（文件页列表、全部文件、日志审计）天然恒在顶部，无需 sticky；其余后台/设置各表用**有界高度链**承载滚动（卡片 `flex-1 min-h-0` 落在 `h-screen` 外壳内，公告横幅出现时表格变矮而非把分页挤出视口），表头随内容滚动。粘性样式集中在 `components/ui/sortable-header.tsx` 与各页容器，禁止内联散写；表头底色一律与所在卡片同源（透明）。

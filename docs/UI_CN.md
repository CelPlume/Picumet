...
- 连线：转角是定高 SVG（`M 1 1 L 1 4 Q 1 10 7 10 L 17 10`，圆角半径 6px）接到锚点左缘；祖先列只要「后面还有兄弟」就整行贯通（`pass` 布尔数组随递归下传），故展开子树不会把桶干线切断——竖线用 `top-0 height:100%` / `calc(50% - 10px)` 百分比表达，行高随内容与字号自适应。
- 配色：桶基色按层级做色相旋转（level 0 = 桶基色即其 pill 色，level n = 基色 + 34n°），同桶同族、层级可分辨。
- 桶可折叠（默认展开）、挂载点同时展开（`Set`），备用桶 badge 高亮主桶泳道 3 秒；挂载点视图根容器留 16px 内边距（不贴卡片边），备用桶 badge 与「无挂载点」提示对齐 level-1 内容列且不画连线。

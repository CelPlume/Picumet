...
- Coordinates in lane space: level-0 trunk dot at x=6, level-1 mount dot at x=28 with a 10px fork connector; dots are hollow when collapsed and solid when expanded, with no white ring.
- One guide line per level: trunk segments stop at the last mount dot's center, the level-1 line starts below its dot (a hollow circle never shows a line through it), and folder rows get their own `border-l` guides per depth.
- Buckets collapse (default open), mounts open simultaneously (a `Set`), and the standby badge highlights the primary lane for 3 seconds.

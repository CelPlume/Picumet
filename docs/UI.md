...
- Connectors: the corner is a fixed-height SVG (`M 1 1 L 1 4 Q 1 10 7 10 L 17 10`, 6px radius) meeting the anchor's left edge; an ancestor column is drawn through the whole row whenever it still has a later sibling (a `pass` boolean array threaded through the recursion), so expanding a subtree never cuts the bucket trunk — vertical runs use `top-0 height:100%` / `calc(50% - 10px)` percentages, so row height follows content and font size.
...
- Buckets collapse (default open), mounts open simultaneously (a `Set`), and the standby badge highlights the primary lane for 3 seconds; the mount view root keeps 16px padding (never flush with the card), and standby badges plus the "no mounts" hint align to the level-1 content column without a connector.

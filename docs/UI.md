...
- **Popover surface tier**: `.glass-surface-popover` alpha sits 0.12 below the card alpha (floor 0.5, `hsl(var(--popover) / max(0.5, calc(var(--glass-alpha) - 0.12)))`) — popovers usually float above white surfaces and must read as more translucent than cards for the glass to show; the off tier is covered by the `.no-blur` solid fill.
...
- **Unified header form**: the files page's list-view header and its rows share one constant and one geometry (`item-surface` glass recipe, measured identical on both sides: `font-size 14px`, `padding 8px 16px`, `border-radius 0`, `position sticky`), and list rows use `px-4` so their columns line up with the header; native admin/settings table headers are **transparent with no fill** (short tables do not stick, and long tables get their stickiness from the page structure). The legacy constants `TABLE_HEAD_CLASS` / `TABLE_HEAD_SOLID` and `.glass-header` are deleted — never revive them.
...
**Sticky headers**: every data table (admin users/all files/shares/logs/permissions/storage providers and mounts, settings shares/API keys/access rules) keeps its header pinned while content scrolls, with the implementation following the page form: the files-page list header uses the same `item-surface` recipe as its rows and sticks; admin/settings tables carry stickiness through **bounded scroll areas** (`max-h-[calc(100vh-{margin})] overflow-auto`, each page's margin calibrated against its first screen — settings ≈11.7/12.9rem, logs 10.45rem, all files' pagination bottom = `calc(100vh-12.7rem)`); short tables (providers, permission rules) do not force stickiness. Sticky styling concentrates in `components/ui/sortable-header.tsx` and each page's container — no inline scattering; the standalone low-alpha `.glass-header` recipe is gone (deleted), and header fills always come from the containing card or row.

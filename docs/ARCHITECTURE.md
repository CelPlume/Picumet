...
**Visibility and guest semantics (PERM-09).** `visibility='users'` means every **signed-in principal** (user or API key) may `read` and `download` the file, and step 3 exempts exactly those two actions from the `defaultPath` boundary; `'public'` grants the same to anonymous visitors. `file_metadata.guest_visibility` applies to **anonymous visitors only**: `view` = list + download, `download` = download without listing, `none`/unset = nothing. Anonymous principals get no step-9 fallback, so without a `role='guest'` rule, a mount-matrix guest entry, or one of the explicit public channels (the gallery, a signed direct link, or a public mount) an anonymous listing is denied by design. Visibility and guest visibility only ever relax `read`/`download` — they never relax a write or a path boundary, and a `deny` rule still outranks the synthesized rule.

**Owner fallback and rule precedence over the matrices (PERM-12/13).** Step 6 (owner fallback) runs **before** both matrices, so a matrix `deny` cannot restrict an owner's own file for `read`/`update`/`delete`/`share`/`download`. `write` is deliberately absent from the owner fallback: uploading into a mount whose matrix forbids `write` is still refused. `user`-origin rules (step 5) also run before the matrices and sort `user > system`, so a user-authored allow rule on their own path can outweigh a matrix `deny`. A matrix is therefore a **default-permission fallback layer, not an absolute boundary** — "users may upload but an admin keeps management rights" cannot be expressed with a matrix (use a path rule or a capability bit), and a boundary that must be absolute has to be enforced ahead of the rule and owner layers.
...
**New-user default posture (PERM-01).** A new user's `defaultPath` resolves as explicit argument → `role_defaults.default_path` → `'/'` in `UserRepo.createUser`, and every creation path (registration, free mode, seed) shares that source: editing a role's default path in the admin settings applies to users created **afterwards**, while "save role defaults" still bulk-overwrites existing members. The value is stored verbatim — `default_path` is not a `<username>` template (the placeholder in the admin form is illustrative). The built-in seed keeps `'/'`, i.e. the default posture is a **shared namespace** in which every registered user holds the full role vocabulary and nothing isolates one user's tree from another's. An isolated deployment instead sets the role's default path to a per-user prefix (for example `/users`) and tightens the role matrix. Know the limits of that isolation: outside its root, step 3 denies every **write** hard, before any rule or owner layer, while `users`/`public` files stay `read`/`download`-able through the visibility exemption — isolation restricts the write surface, not the shared read surface.
...
**Password protection (file-level).** Password protection is file-level only. A protected file answers `403 PASSWORD_REQUIRED` on every content outlet until its access password is verified through `POST /api/files/{id}/verify-password`, which requires the `download` permission on the file (same gate as the download-link endpoint) and issues a gateway token carrying the `passwordVerified` flag on success. Rule condition fields (`requirePassword`, `passwordHash`, IP conditions) are no longer accepted at rule creation; the engine keeps failing closed on legacy rules that still carry conditions — such rules deny every request unless explicit conditions are supplied, and the only production caller that does so is the password-verify flow (with the real client IP and `passwordVerified: true`).
...
**Synthetic root (TOP-16).** A topology without a mount covering `/` — for example only `/storage1`, `/storage2`, `/storage3` — still lists. When `findMountForPath` finds no mount but mounts exist below the requested path, `GET /api/files` and `GET /api/files/tree` aggregate the top-level mounts into virtual directories from `services/storage/root-view.ts` (`resolveVirtualListing` / `resolveVirtualTree`), and the public browser, AList `fs/list` and WebDAV `PROPFIND` synthesize the same virtual folder set in their own item shapes. On the file page a virtual item is a folder with `id = 'vroot:<full path>'` and **no `file_metadata` row**, so it cannot be shared, moved, renamed or deleted — every row-addressed operation answers 404. Read is gated per mount root through the pure `checkPermission(..., 'read')` with that mount's rules and matrix preloaded once per request (no per-row queries); an intermediate virtual directory appears when at least one mount below it is readable, and a level where nothing is readable stays `404` so the mount topology is not disclosed. Writes are never synthesized: upload, folder creation, detail and the remaining row-addressed endpoints answer `404` on a virtual path (a root upload has no mount and is refused).
...
- Reference release happens inside the same SQL batch as the row delete/rewrite (`NOT EXISTS (SELECT 1 FROM file_metadata WHERE blob_hash = ?)` — no refcount column, no read-modify-write race); the last reference enqueues the object in `blob_gc`, where a scheduled task deletes it after a grace period, re-checking references and retrying on failure. Both the index and the GC queue are keyed by **`(hash, mount_id)`** (deduplication is per mount, cross-mount copies are stored separately) so identical content in different mounts cannot overwrite each other's queue entries; moving a blob file across mounts currently returns `422` (its physical object and source-mount index are bound together — metadata-only moves would tear the capacity and GC boundaries).
- Multipart upload sessions stay path-keyed (`blob_hash` NULL): parts go straight to the provider, so their objects remain exclusive and the cleanup removes them directly. Completion trusts, in order, the server-recorded parts → the client-reported parts → the bucket's `ListParts`, so pre-signed direct uploads still merge correctly when the bucket CORS does not expose ETags; `upload-complete` **claims the session atomically** with a conditional UPDATE (only one winner among concurrent completions, a crashed claim is taken over after 5 minutes).
- Quotas keep counting logical sizes (each file counts its own size), so capacity gates stay conservative.

**Read-path failover (§G).** Object reads go through `serveFileObject` / `getFileObject` in `services/storage/failover.ts`. When the bucket recorded for a file cannot return the object (404) or the upstream fails (502 / `ProviderError`), the read falls through to the other pool members of the same mount in order — KV location hint → the file's recorded provider → remaining members by weight — capped at four candidates, with an 8-second limit per attempt so an unreachable bucket cannot stall the request. Three hardening rules: **circuit breaking** — an upstream-failing bucket gets `pool:down:<providerId>` (45-second TTL) and moves to the end of the candidate list for that window (it keeps one last-resort attempt, which doubles as the recovery probe) and the marker is cleared on the first success; a missing object never trips the breaker (data state, not bucket health); and **content validation** — a hit on a bucket other than the recorded one is checked against the metadata `size` first (skipped when either side has no size), and a mismatch counts as a miss so stale or truncated mirror content is never served, while etags stay out of the decision because their semantics differ per backend. A replica hit stores `serve:loc:<fileId>` (provider id plus a physical-key fingerprint, 10-minute TTL, refreshed on every hit) so later reads prefer that bucket; rewriting the file invalidates the hint through the fingerprint, and the recorded provider serving again clears it. **Mirror exemption:** old-object cleanup and blob GC only ever delete from the file's recorded provider (or the provider registered in the blob index), never from other pool members, whose mirror content belongs to the external sync channel. Cross-bucket **copy** is not part of this layer: replicating objects into secondary buckets belongs to the future Go backend. The admin surface plans a per-mount "automatic cross-bucket sync" switch that is only selectable in a Go-backend environment — the current Node/Workers backend does not implement the capability (control rendered disabled); pooling (§E) and read failover (§G) work without it. Callers that pass the request principal's role and the read action (`read` / `download`) make the candidate loop re-evaluate the bucket (§31) and mount (§28) matrices per candidate, skipping a `deny`ed bucket exactly as the write path skips one that cannot fit — so a fallback read cannot reach a bucket the role is not allowed to read from; internal callers that pass no role skip the check and keep the legacy behaviour.
...
**Write-entry modes and the default role matrix (§28).** Mounts gain two capabilities aimed at public upload areas. `mounts.upload_mode` constrains write paths: `free` (default, current behaviour), `user_space` (writes are forced into `<mountPath>/<username>`, created on first use — usernames are unique, so users never collide, and a renamed user keeps the old directory in place) and `flat` (folder creation is rejected inside that mount, uploads stay in one layer; a name already taken still answers the existing 409 "target path is occupied by another user" — naming uniqueness comes from `object_key`, the mount-relative path, while content hashing only deduplicates physical objects). `mount_role_permissions` holds the per-mount role matrix (`role` ∈ admin/user/guest, action vocabulary read/write/update/delete/download): it is evaluated after the file-owner fallback and before the role defaults, a present entry is a **closed set**, no entry falls back to the role defaults, `share` stays out of the matrix (sharing is the `can_share` capability bit), and explicit `path_rules` of any origin always win. The constraints live in the pre-write checks of `files/write.ts`, so `/api/upload`, the compat upload endpoint, WebDAV PUT/MKCOL, the S3 gateway and AList all behave identically.

**Pooled-mount placement strategies (§29).** The write-path bucket choice (`mounts.pool_strategy`) grows to five options: `least_used` (smallest usage, scored `(used+1)/weight`), `round_robin`, `hash` (**directory-sticky** — hashes the parent directory so one directory lands in one bucket and prefix listings stay contiguous; a fixed member set always maps the same directory to the same bucket), `free_weighted` (`(member capacity − used) × weight` wins; a `mount_providers.capacity_bytes` that is unset counts as unlimited and outranks any finite headroom; **all members full → 413**, failing closed instead of silently overfilling), and `ordered` (fills members by `mount_providers.sort_order`, moving on only when the current one is full; without a capacity the first member is always chosen). `weight` and `sort_order` have distinct jobs: the former weights `least_used`/`free_weighted`, the latter queues `ordered`. All five only affect new writes — reads and deletes resolve through `file_metadata.provider_id`, so existing objects never migrate. The admin mount form selects the strategy and sets a capacity cap plus an upload order per pool member.

**A member's capacity is a hard cap (§30).** "Is this member full right now" is not a sufficient test: the check must include **the incoming object size** and **in-flight reservations** — `used (aggregated from file_metadata) + mount_providers.quota_reserved + incoming size <= capacity_bytes` (`NULL` = unlimited, which neither fills up nor reserves). Reservations go through a conditional atomic UPDATE; zero affected rows means that member cannot take the object right now, so the next candidate in the strategy's order is tried and a 413 is only raised once none of them fits. The strategy therefore decides **candidate order**, never whether capacity can be exceeded (`hash` stickiness yields when a directory's bucket is out of room). The reservation lifecycle matches the mount-level `quota_reserved`: it is released on write success, failure and compensation, and on upload-session completion or expiry.

**Bucket-scoped role matrices and an explicit standby flag (§31).** The default role matrix can now be configured **per bucket** (`mount_provider_role_permissions(mount_id, provider_id, role)`), resolved as **bucket → mount (§28) → role defaults**; with neither entry the behaviour is unchanged. Actions whose bucket is already known (read, update, delete, download, share) are judged against the **file's actual bucket**; a write is judged inside the placement **candidate loop**, where a bucket whose matrix forbids the action is skipped exactly like a full one — so only a 403 once every candidate refuses, and a strategy still decides candidate order alone. `mount_providers.standby` makes "serves as a standby bucket" **explicit**, and the old "zero files for this mount" inference has been **removed entirely** (a flagged bucket shows up as a standby even when it holds files, and an unflagged member with zero files is no longer treated as one). In the mount view a standby lane renders one node row per standby mount (amber hollow ring, not expandable, not connected to the trunk) and clicking it locates the mount row on the primary lane. Capacity has two hard layers: per bucket `mount_providers.capacity_bytes` (placement, §30) and the mount total `mounts.max_storage` (write quota); the total must not exceed the sum of the bucket caps (uncapped buckets are excluded from the sum) or the API answers 400.

**Mount and provider lifecycle constraints (TOP-01/02/03/07/08/11).** Configuration writes enforce the invariants the resolver depends on, and all of them fail closed:

- **One path, one mount.** Provider one-step creation, `POST /mounts` and `PUT`/`PATCH /mounts/:id` share one placement validator; a second mount on the same normalized path answers `400 ALREADY_EXISTS`.
- **Nesting requires `ancestor.priority <= descendant.priority`** (equal is legal, depth breaks the tie), because a parent with a strictly higher priority would shroud the child subtree and make it unreachable. A violating combination answers `400` with the explanation; a new nested mount defaults to the highest priority among the mounts covering it.
- **No overlay.** Creating a mount — or re-pathing one — where the parent mount already holds `file_metadata` rows at that path or below it answers `409`: shadowing would leave the parent's objects unreachable while still counting against quota. Migrate the data first.
- **Mount-point directory rows are system rows.** The row lives in the parent mount's namespace under the deterministic id `mountfolder:<mountId>`; a user folder or file on the same display path is **never reused** (admin create/update answers `409`, the background self-healing paths warn and skip), and removal deletes by that id only, so dropping a mount can no longer delete a user directory. Its owner is the oldest administrator, purely as a foreign-key placeholder.
- **Provider deletion checks every reference** — anchors, pool members, landing rows, in-flight upload sessions, the content-hash index, the blob GC queue, and bucket matrices. Any hit answers `409` with the per-category counts and changes nothing; the pool-member and bucket-matrix foreign keys are `ON DELETE RESTRICT` as a second line of defence.
- **Removing a pool member is refused** (`409`, with per-bucket file and session counts) while files or in-flight upload sessions still land on that member. Objects are never migrated automatically.
- **Config saves preserve in-flight reservations.** Member replacement is a differentiated upsert that leaves `mount_providers.quota_reserved` and `created_at` untouched and deletes only members genuinely absent from the new set, and the whole mount configuration write (mount row, upload mode, role matrix, members, bucket matrices, anchor, directory row) runs in **one transaction** after a separate read/validation phase. The scheduled reconciliation rebuilds the user, mount **and per-member** reservation layers from the in-flight upload sessions, so a reservation that drifted (for example through the historical delete-and-insert member replacement) converges on the next pass.
...
**Responsibilities**: expired quota release, move-source cleanup, expired share marking, quota reconciliation, content-object GC and index reconciliation, multipart-abort retries and audit-log cold archiving, run by a scheduled task (`crons = ["*/10 * * * *"]` — every 10 minutes).

- `releaseExpiredReservations` releases quota reservations held by expired upload sessions and aborts their provider-side multipart uploads (failures are queued with their `upload_id` for retry).
...
- `reconcileQuotas` corrects the `used_storage` / `used_files` counters and recomputes the three reservation layers from both in-flight sessions and the `quota_reservations` ledger.
- `reconcileBlobs` / `cleanupBlobObjects` repair content indexes and reclaim content objects per `(hash, mount_id)`.
- `cleanupUserDeletedObjects` / `cleanupMultipartAborts` drain the `orphan_objects` queue (deleted users' objects, failed multipart aborts).
- `archiveAuditLogs` exports whole-hour audit windows older than the retention period (`audit_retention_days`) to the R2 cold archive (`AUDIT_BUCKET`), writes the manifest and rollups in one batch after verification, then prunes hot rows in batches; without the bucket it only reads and never deletes.
- `repairMovedFolderOrphans` heals subtree orphans left by historical cross-mount folder moves; `ensureAllMountFolders` backfills mount-point directory rows.
...
| `mount_role_permissions` | Per-mount default role permission matrix (§28): `(mount_id, role)` primary key plus `permissions` (comma-separated action vocabulary). A present entry is a closed set; without one the role defaults apply. |
...
| `blob_gc` | Content-object collection queue (primary key `(hash, mount_id)`): enqueued when the last reference disappears, deleted by a scheduled task after a grace period, retried on failure. |
| `upload_sessions` | Tracks upload progress, parts, and reserved quota; `complete_claimed_at` is the atomic completion-claim marker. |
| `quota_reservations` | Member-level reservation ledger for direct writes and cross-mount moves, merged by reconciliation. |
| `audit_archives` / `audit_rollups` | Audit cold-archive manifests (range, row count, object key, SHA-256) and per-hour counters for archived windows (merged with the hot table by the trend endpoint). |
...
- **Public CDN mounts**: configuring `publicDomain` on a provider makes **every object of its mounts** anonymously readable straight from that origin, bypassing the guest master switch, visibility, guest visibility and both matrices (see the deployment guide). Treat it as a deliberate public-exposure switch, not a convenience setting.
